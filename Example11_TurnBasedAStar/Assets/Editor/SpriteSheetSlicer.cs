...
public class SpriteSheetSlicer : EditorWindow
{
    ...
    public static void SliceUsingXML(MenuCommand command)
    {
        ...
        SpriteSheetSlicer window = ScriptableObject.CreateInstance<SpriteSheetSlicer>();
    ...
    public SpriteSheetSlicer()
    {
        title = "Sprite Sheet Atlas Slicer";
    }
            ...
            foreach (XmlNode childNode in root.ChildNodes)
            {
                if (childNode.Name == "SubTexture")
                {
                    try
                    {
                        ...
                        int y = textureHeight * 2 - (height + Convert.ToInt32(childNode.Attributes["y"].Value));
